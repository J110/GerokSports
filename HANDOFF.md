...
**Next session's first action: Phase 1 second-pillar RE-SELECTION (Recent-Overs candidate also DEFERRED to Phase 3 UI scope per `b1a4ad8`-tier finding)** — C29b retired (FALSIFIED — `d40e2b9`); WS-Surface-E retired (detection-layer falsified — `a4f91f5`); WS-K retired (state-machine layer falsified at universal-candidate-exhaustion — `8f79b03`). Recent-Overs / B-γ classification (HANDOFF line 725 standing classification: "out of scope for SM/pipeline workstream") confirms Phase 1 second pillar must select from alternative candidates: **§11.3 item 4 (Per-batter-ledger conservation)** OR WS-D §3.5 Layer 1a (HE survivor from C29b — pipeline-side, S12 plumbing concern, 1-2/5 budget) OR WS-F bowler-misattribution (narrow scope). F1017 phantom-wicket cohort REMAINS as architectural-known-defect at every pipeline-internal layer per surface_pair §2.10 — future re-investigation requires (1) cohort growth via natural production OR (2) Scout primitive expansion to include signal not downstream of strip OCR OR (3) KF Cricbuzz real-time corroboration (Phase 2 architectural pivot) OR (4) operator-side retraction workflow. KF deferred to Phase 2 queue. S23-corollary + S23-extension Phase 4 catalogue items unchanged. S27 candidate (shared-signal-source structural barrier) surfaced — single instance, awaits second-instance confirmation.
