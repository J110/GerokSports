...
**Last update**: 2026-05-24 (**WS-Q ARC FULLY CLOSED — step-3 arc close-out commit**; multi-commit refactor delivered as predicted; cumulative WS-Q trajectory **59 → 34 divergences (-42% closure at step-2c; -83.6% from original 207-row baseline)**; **S9-strict PROMOTED** to numbered methodology insight (3rd-instance: F1 + S18 + WS-Q canonical-store-flag-flip; architectural cure closes multiple defect classes simultaneously — E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline closed at step-2c `88fe41c`); **FA forward-compat empirical validation** landed as candidate insight (first-instance: WS-V.A1 step-2 §10 prediction confirmed via cross-fixture audit at `4e3b3d3` — `PREV-SCORE-ANCHOR-APPLIED` count = 0 across 4 captures + 2,472 trace records post-step-2c flag flip); **architectural-cure-over-defensive-iteration economics** landed as candidate insight (first-instance: WS-Q -25 rows / ~107 LOC vs prior 3 defensive no-ops WS-O OA + WS-P P1 + WS-U V2 = 0 rows / ~150-200 LOC similar effort). Validated baseline NOW = **34** (was 59 post-WS-V Phase 1; before that inflated 93). All future predicted-flip tables compare against 34. Methodology insights running total **27 → 28** (S9-strict promoted) + 2 candidates (FA forward-compat + architectural-cure economics). Phase 4 catalogue updates: E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline RETIRED via WS-Q cascade closure; S12 (eyes/main.py threading) ADDED from WS-Q step-2d deferral; WS-V.B.W + WS-V.B.Z + F1017 stay catalogued.

**Budget discipline removed by user direction 2026-05-24.** Empirical-falsification budget counters and methodology cap framing retired going forward. Execute decisively. Discipline retained: code structure + planning + quality gates (L1.5 + L2 + derivation green) + single-IV for empirical replays (causal attribution) + honest scope assessment. Historical budget references in past memos preserved as factual records.
...
**Next session's first action: highest-leverage residual cohort selection.** WS-Q arc fully closed; remaining residue is independent of the score-cascade root WS-Q targeted. Candidates (prioritize by leverage):
1. **F-A-commit-lag (10 rows)** + **F-B-ad-occlusion (8 rows)** — bowler-state commit-lag + ad-occlusion residue; investigate root cause; potentially shares architectural surface.
2. **WS-V.B.W step-1c** — 10-row Cohort W at 4.3 true pipeline cricket-defect (striker_name vs stats internal inconsistency); Phase 4 catalogued; step-1c instrumentation is the entry data.
3. **Track 2 re-enable** (OpenScout `USE_OPEN_SCOUT=0`; oldest deferral; high-leverage).
4. **S12 follow-up** — eyes/main.py `apply_scorer_decision` SM threading from WS-Q step-2d deferral (low-priority cleanup).
5. **Recent-overs-drop (4 rows)** — Phase 3 UI render-layer scope per standing classification.

All future predicted-flip tables compare against validated baseline **34** (NOT 59 or inflated 93) per WS-Q step-2c architectural-cure closure. F1017 phantom-wicket cohort remains architectural-known-defect within broadcast framework per retirement directive at `9653cff`. S25 + S27 + S33 (independent third-instance) + replay-tool-fitness promotion candidates remain awaiting confirmation thresholds. New candidates landed at WS-Q step-3: FA forward-compat empirical validation + architectural-cure-over-defensive-iteration economics (both first-instance; await second-instance for full promotion).
...
| WS-L step 3 | `70eb977` | docs(workstream-l): step-3 close-out — primary CLOSED (assertion shipped + cohort discriminated) + S28 promotion + WS-M opener | Memo §15-§18 appended (Fork B retirement formalized + S28 promotion to numbered insight with two-instance evidence + S26-v2 candidate first-instance footprint + WS-M handoff); HANDOFF + Architecture_HANDOFF status; methodology insights 23 → 24 (S28 promoted); next-session retargeted to WS-M step-2 patch authorization |
| WS-M step 1 | `cfbd2f5` | docs(workstream-m): step-1 investigation memo — orphan-bind pipeline-plumbing audit; 4 LIVE-FAIL cohort inherited from WS-L; MA leading with sub-mechanism reservation | 257-line step-1 memo; pre-screen GREEN-PIPELINE-PLUMBING-REQUIRED per S28; MA leading with sub-mechanism reservation; MB/MC/MD/ME statically falsified or out-of-scope; S26-v2 candidate second-instance footprint emerging |
| WS-M step 1b | `c46fcb0` | docs(workstream-m): step-1b sub-investigation — cohort splits per sub-mechanism 3+1 | Memo §11-§13 appended (per-fixture orphan-bind lifecycle inspection: 22+6+6+0 ORPHAN events across 4 fixtures); cohort SPLITS 3+1 (Sub-cohort A 3 dckkr fixtures queue-empty class + Sub-cohort B 1 fixture non-orphan-bind class deferred to Phase 4); Shape 2 consumer-side fallback bind recommended; S26-v2 second-instance achieved (10× cost reduction demonstrated) |
| WS-M step 2 | `d2f0759` | feat(workstream-m): M1 Shape 2 — consumer-side fallback bind closes Sub-cohort A (3 dckkr fixtures, 18-run total) | Patch at score_manager.py:1436-1462 + PENDING-BALL-ORPHAN-FALLBACK-ENQUEUED tag registration + 5 L1.5 cases (M-1 through M-5 in test_orphan_fallback_bind.py); L1.5 75 → 80; strict-extension semantics (fires only when self.striker non-None; existing orphan tag preserved as fallback-fail signal); honest closure caveat documented (runs_delta=0 dot-ball assumption) |
| WS-M step 3 | (empirical replay; no commit) | validate_ws_m_step3_20260523_221808 trace generated via SM-only replay against validate_dckkr_20260521_070545 | Mechanism-confirmation FAIL: PENDING-BALL-ORPHAN-FALLBACK-ENQUEUED count = 0 + PENDING-BALL-SLOT-BOUND-ORPHAN count = 0 (replay tool inadequate — SM-only path bypasses over_mgr.attach_score_manager wiring). trace_alpha_batter_runs_sum SKIPped via Shape B (replay-class trace lacks ui_after). No regression on γ/η/D-post-FoW assertions. **Budget 2/5 → 1/5** consumed per decision-tree letter; root attribution = replay-tool inadequacy, not Shape 2 falsification |
| WS-M step 4 | `b11f000` | docs(workstream-m): step-4 arc close-out — Shape 2 shipped + S26-v2 promotion + replay-tool-fitness candidate + Phase 1 chain complete; budget 1/5 methodology cap proximity | Memo §15-§20 appended (step-3 outcome + arc closure declaration + S26-v2 promotion + replay-tool-fitness candidate first-instance + Phase 4 catalogue entries + Phase 1 → Phase 4 transition framing); HANDOFF + Architecture_HANDOFF status; methodology insights 24 → 25 (S26-v2 promoted); next-session retargeted to Phase 1 → Phase 4 transition decision (assertion-side-only Phase 1 + Phase 4 docs cleanup + natural-session-driven validation) |
| Runbook fix | `3e8ac11` | docs(runbook): fix flag-name drift (--scout-dump → --dump) + add --session-id requirement + clarify --snapshot-output mode coverage (SM-only) | Runbook §5 quick-start + §8 fix-iteration corrected to match argparse; new SM-only-coverage caveat documenting cross-module wiring fix scope (over_mgr ↔ score_manager) NOT covered by runbook workflow; smoke test verified all 3 commands execute correctly post-fix |
| WS-N step 1 | `3914f70` | docs(workstream-n): step-1 investigation memo — snapshotter full-pipeline integration; closes SM-only measurement gap from WS-M step-3 + runbook fix | 339-line step-1 memo; pre-screen GREEN-PIPELINE-DIRECT per S28; NA leading with sub-mechanism reservation; ~5-10 LOC initial estimate (sub-mechanism a hypothetical) |
| WS-N step 1b | `6c322c5` | docs(workstream-n): step-1b sub-investigation — sub-mechanism (b) confirmed; external BallEventDetector wiring required; step-2 scope expansion review needed | Memo §12-§15 appended; sub-mechanism (b) CONFIRMED via code-reading (no `over_mgr.on_ball_event` in score_manager.py; exclusive production-only invocation at test_pipeline.py:9236/13098/14017); revised ~15-25 LOC; S26-v2 third-instance footprint |
| WS-N step 1c | `f8f11b1` | docs(workstream-n): step-1c sub-investigation — Fork C extraction MESSY-PARTIAL (~70-115 LOC including per-frame state mirror); recommend Fork D defer per methodology-cap proximity | Memo §16-§18 appended; Fork C extractability analysis; setup-portion CLEAN (~40-65 LOC) BUT `_pending_bcast_striker_key` per-frame state mirror MESSY (main-loop-local; ~30-50 LOC regardless); revised ~70-115 LOC; S26-v2 fifth-instance footprint |
| WS-N step 2 verification | (no commit) | feat(workstream-n): N1 Fork C — (DEFERRED at verification step 1) | Verification step 1 surfaced setup-block-not-discrete (closures bind primitives to main-loop state); revised ~90-165 LOC across 2-3 commits; S26-v2 sixth-instance footprint; user pivoted to Phase 2 reframe |
| WS-N Phase 2 reframe | (prior) | docs(workstream-n): Phase 2 reframe — full-pipeline snapshotter integration moved to Phase 2 backlog; six-layer S26-v2 cascade + 10x scope growth + scope-expansion-cascades-as-Phase-2-signal candidate | Memo §19-§22 appended (Phase 2 reframe declaration + scoping artifact pointer + candidate methodology insight + status footer); HANDOFF + Architecture_HANDOFF Phase 2 backlog framework established; WS-N **PHASE-2-REFRAMED** (not closed/retired; active backlog with full scoping); methodology insights running total 25 unchanged |
| WS-V.A1 + WS-V.B Phase 1 | `c7c853e` | fix(ingest-gt): Phase 1 GT-encoding-convention-fix — match pipeline eager-rotation; closes Y cohort 34 rows + WS-V.B arc close-out | Ingester-only patch at `files/scripts/ingest_cricbuzz_ground_truth.py:547-553/686-691` (lazy → eager end-of-over rotation); Y cohort 34 rows + 1-row boundary cascade CLOSED; H3 baseline calibration META-FINDING CONFIRMED (diff baseline over-counted pipeline defects by 34-45 rows across entire post-WS-V.A1 iteration arc); validated baseline 59 (was inflated 93); WS-V.B memo §11-§14 + WS-V.A1 memo arc-close section in place |
| WS-V step-4 arc close-out | `999ebbf` | docs(workstream-v): step-4 combined arc close-out — WS-V.A1 + WS-V.B CLOSED; S34 PROMOTED + S35 PROVISIONAL + S33 2nd-instance + H3 baseline calibration; validated baseline 59 (was inflated 93) | HANDOFF + Architecture_HANDOFF promote S34 + S35 to standing methodology; H3 baseline-calibration mandate landed verbatim; WS-V arc 207 → 59 (-71%); §17.13 retrospective added; Phase 4 catalogue extended with WS-V.B.W (cohort W mid-over 4.3 true pipeline defect) + WS-V.B.Z (cohort Z mid-over 4.4 acceptable residue, possibly re-verifiable); methodology insights running total 25 → 27 (S34 + S35 promoted; S33 stays 2nd-instance footprint awaiting independent third instance); next-session retargeted to WS-Q score-axis step-2a (foundation refactor behind `USE_SM_CANONICAL_SCORE=0`) |
| WS-Q step-2a | `d6f041b` | feat(workstream-q): step-2a foundation — sm._canonical_score storage + sm.set_score canonical API + USE_SM_CANONICAL_SCORE env gate (default 0, zero behavior change) | Foundation infrastructure under `USE_SM_CANONICAL_SCORE=0` (default; zero behavior change); 25 logic LOC ≤60 budget; 5 L1.5 cases (Q-1/Q-2/Q-3/Q-4/Q-4b); diff baseline 59 UNCHANGED; S34 pre-patch verification 5/5 PASS |
| External-corroboration retirement docs | `9653cff` | docs: retire external-corroboration framing per user direction 2026-05-24; broadcast IS the corroboration | Standing directive in HANDOFF + Architecture_HANDOFF; KF Phase 2 deferral queue retired; F1017 stays architectural-known-defect within broadcast-reading framework; Cricbuzz commentary fixture role narrowed to offline regression GT only; 8 LOC delta ≤10 budget |
| WS-Q step-2b | `a5c8712` | feat(workstream-q): step-2b writer redirect — 4 production sb.set score writers route through sm.set_score canonical API + SM-SHADOW-PARITY-DIVERGENCE logging (flag still 0; observability only) | 4 explicit redirects (S07 + S08-guarded + S08-direct + S09) + setter S01 parallel call; flag stays default 0; SM-SHADOW-PARITY-DIVERGENCE surfaced **6 events** on dckkr at frames 288/290/303/319/334/361 (canonical 43/44/45/49 vs sb stuck 63/64 — empirical signature of WS-O.c cascade root); 27 logic LOC ≤150 budget; diff baseline 59 UNCHANGED; 3 new L1.5 cases (QB-1/QB-2/QB-3) |
| WS-Q step-2c | `88fe41c` | feat(workstream-q): step-2c flag flip + sm.set_score regression-rejection predicate (cricket-physics + confidence); WS-O.c residue closure via canonical store authoritative reads — diff baseline 59 → 34 | **THE LOAD-BEARING COMMIT.** Flag flip default 0 → 1; T20 absolute bound + forward-monotonic + retroactive-correction-with-confidence predicate at `set_score`; Phase 1 empirical anchor 6/6 GT match at balls 4.1-4.6; **diff baseline 59 → 34 (-25 rows, -42%)**; cascade closure E2-phantom-runs (3→0) + Per-batter-ledger-drift (5→0) + phantom-in-pipeline (3→0); FA self-disabled (PREV-SCORE-ANCHOR-APPLIED 15→0); 57 logic LOC ≤80 budget; 6 new L1.5 cases (QC-1 through QC-6) |
| WS-Q step-2c.5 audit | `4e3b3d3` | docs(workstream-q): step-2c cross-fixture audit baseline replay — gate-7 verification before step-2d cleanup authorization | Cross-fixture audit across 3 dckkr captures (264/749/618 records) + gtrr (841 records); verdict GREEN-WITH-LIMITATIONS; FA count = **0 across 4 captures + 2,472 trace records** (forward-compat contract empirically validated); no new defect classes; PA preserved as defense-in-depth (3/9/3/8 cross-fixture); audit memo at `files/docs/investigations/workstream_q_step2c_cross_fixture_audit.md` |
| WS-Q step-2d | `24b9777` | refactor(workstream-q): step-2d cleanup — legacy sb.set retirement + FA explicit retirement + S07/S08/S09 redirect consolidation (post-step-2c architectural cure) | Cleanup: legacy sb.set calls retired at S01 setter + S07 + S08 + S09 (sm.set_score sole writer); FA block at `score_manager.py:4234-4268` retired + PREV-SCORE-ANCHOR-APPLIED tag retired from KNOWN_TAGS; PA + retroactive-correction predicate branch preserved as defense-in-depth; S11 + S12 deferred; net -31 LOC production with only +7 new logic ≤80 budget; diff baseline 34 UNCHANGED; 2 new L1.5 cases (QD-1/QD-2) + QB-3 updated |
| WS-Q step-3 arc close-out | (this commit) | docs(workstream-q): step-3 arc close-out — WS-Q CLOSED (207→34, -83.6%); S9-strict promoted (3rd instance); FA forward-compat + architectural-cure-economics candidates landed | HANDOFF + Architecture_HANDOFF promote S9-strict to numbered methodology insight (3rd-instance evidence: F1 + S18 + WS-Q); FA forward-compat empirical validation + architectural-cure-over-defensive-iteration economics landed as candidates (first-instance each; await second-instance); §17.14 WS-Q arc retrospective added; validated baseline = **34** going forward; Phase 4 catalogue updated (E2 + Per-batter-ledger-drift + phantom-in-pipeline RETIRED via WS-Q; S12 ADDED from step-2d deferral); methodology insights running total 27 → 28 (S9-strict promoted) + 2 candidates; next-session retargeted to highest-leverage residual cohort selection |
...
## Phase 2 backlog (active; budget-recovery-gated)

Workstreams reframed as Phase 2-class architectural-refactor (not Phase 1-affordable surgical-fix). Active backlog with canonical scoping artifacts; execution triggers on cohort accumulation OR empirical-budget recovery to ≥3/5.

| Workstream | Phase 2 trigger | Scoping artifact |
|---|---|---|
| **WS-N — Full-pipeline snapshotter integration** | (a) ≥3 workstreams blocked on cross-module-fix verification via runbook OR (b) empirical-budget ≥3/5 | `files/docs/investigations/workstream_n_snapshotter_full_pipeline_integration.md` §1-§21 (canonical Phase 2 scoping; comprehensive 6-layer S26-v2 cascade analysis + Fork A/B/C/D cost-benefit + extractability classification + `_pending_bcast_striker_key` per-frame state mirror requirements) |

**Operational mitigation during Phase 1.** Runbook SM-only caveat at `3e8ac11` documents the limitation explicitly. Production-session-driven validation mode (per WS-M step-4 standing discipline) handles cross-module fix verification need via natural session traces + shipped assertion re-run at zero budget cost.

---

## §17.14 WS-Q ARC FULLY CLOSED — step-3 arc close-out retrospective

WS-Q multi-commit arc closed at this step-3 docs commit after a 5-commit execution (4 production + 1 cross-fixture audit). The "empirical residue-closure gate" framing at WS-Q step-1 (`a3e75d2`) delivered exactly as predicted: dual-state-write defect class targeted at score domain, canonical store authoritative, WS-O.c residue closed. Cumulative empirical outcome: **207 → 34 divergences (-83.6%) from original baseline** with **-25 rows (-42%) closure attributable to WS-Q step-2c specifically**.

**Arc trajectory (5 commits + 1 retrospective).**
1. step-2a `d6f041b` — foundation (`sm._canonical_score` + `sm.set_score` API + env gate; flag=0 default; zero behavior change).
2. External-corroboration retirement docs `9653cff` — sequenced before step-2b to clear stale framing.
3. step-2b `a5c8712` — writer redirect at 4 sites + SM-SHADOW-PARITY-DIVERGENCE observability; 6 divergences surfaced on dckkr at balls 4.1-4.6 (the WS-O.c residue empirical signature).
4. step-2c `88fe41c` — **THE LOAD-BEARING COMMIT**: flag flip + regression-rejection predicate (cricket-physics + confidence); diff baseline 59 → 34.
5. step-2c.5 `4e3b3d3` — cross-fixture audit GREEN-WITH-LIMITATIONS; FA count 0 across 4 captures + 2,472 trace records.
6. step-2d `24b9777` — cleanup (legacy sb.set retired + FA retired + PA preserved + retroactive-correction predicate preserved).

**Cascade-closure attribution (multi-defect simultaneous closure at step-2c).**

| Defect surface | Pre-step-2c | Post-step-2c | Closure mechanism |
|---|---|---|---|
| E2-phantom-runs | 3 | **0** | Canonical store reads correct 43/44/45/49 at balls 4.1-4.6 (vs sb stuck 63/64); derivation deltas correct. |
| Per-batter-ledger-drift | 5 | **0** | UNPREDICTED CASCADE: batting-card delta calculations align with GT once score reads correct; the 5 drift rows were downstream of the score-cascade root. |
| phantom-in-pipeline | 3 | **0** | Per-ball UI snapshots emit at correct score values; no phantom-event fabrication. |

**S9-strict PROMOTED (3rd-instance evidence threshold met).** Canonical statement: "Single architectural fix closing multiple defect classes simultaneously. Three-instance evidence at promotion: F1 cascade-closure (cold-start striker fix closed N classes); S18 composite-fix (D1+P1 closed γ-bowler-w residue); WS-Q canonical-store-flag-flip (closed E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline simultaneously per step-2c `88fe41c`). **Pattern: when defensive iterations saturate against persistent residue, root-architectural-fix is the next layer to investigate — not more defensive layers.**" S9-strict joins §15 + S21 + S23 + S26 + S26-v2 + S28 + S34 + S35 as standing methodology infrastructure.

**FA forward-compat empirical validation (candidate insight; first-instance via this arc).** WS-V.A1 step-2 §10 predicted FA self-disable post-WS-Q step-2c flag flip; cross-fixture audit at `4e3b3d3` confirmed **0 firings across 4 captures + 2,472 cumulative trace records**. Step-2d retired FA cleanly without revert-cycle cost. Candidate statement: "Tactical defensive patches CAN self-disable structurally when architectural cures land, IF designed with forward-compat contract at commit time." Awaits second-instance for full promotion.

**Architectural-cure-over-defensive-iteration economics (candidate insight; first-instance via WS-O.c residue closure).** WS-Q's 3-commit arc (~107 LOC) closed -25 rows on WS-O.c residue where prior 3 defensive attempts (WS-O OA + WS-P P1 + WS-U V2; combined ~150-200 LOC) closed 0 rows. Candidate statement: "When defensive iterations saturate (3+ consecutive empirical no-ops within same defect class), the next investigation move should be architectural root-cause identification, not additional defensive layers." Trigger: empirical no-op pattern at step-3 baseline replays across 3 workstreams targeting the same residue. Awaits second-instance for full promotion.

**Methodology insights running total: 27 → 28 (S9-strict promoted)** + 2 candidates (FA forward-compat + architectural-cure economics). S25 + S27 + S33 (independent third-instance) + replay-tool-fitness promotion candidates remain awaiting confirmation thresholds.

**Phase 4 catalogue updates.**
- **RETIRED via WS-Q step-2c cascade closure:** E2-phantom-runs (3 → 0), Per-batter-ledger-drift (5 → 0; was Phase 4 catalogued), phantom-in-pipeline (3 → 0).
- **ADDED at WS-Q step-2d deferral:** S12 (`eyes/main.py:apply_scorer_decision` SM threading — function signature lacks score_manager; threading is moderate scope-creep; defer to follow-up).
- **STAY catalogued:** WS-V.B.W (10-row Cohort W mid-over 4.3 true pipeline defect), WS-V.B.Z (10-row Cohort Z mid-over 4.4 acceptable residue), F1017 phantom-wicket (architectural-known-defect within broadcast framework).
- **Validated baseline now = 34 divergences** (cumulative WS-Q trajectory: 207 → 93 inflated → 59 validated → 34 architectural-cure). All future predicted-flip tables compare against 34.

Full retrospective in `files/docs/investigations/workstream_q_score_state_unification_refactor.md` §11-§15 + cross-fixture audit memo at `files/docs/investigations/workstream_q_step2c_cross_fixture_audit.md`.

---

## §17.13 WS-V ARC FULLY CLOSED — combined step-4 retrospective (WS-V.A1 + WS-V.B)

WS-V arc closed at the combined step-4 docs commit (this) after a 5-layer iteration trajectory spanning WS-V step-1 → WS-V.A → WS-V.A1 → WS-V.B step-1 → WS-V.B step-2 (falsified) → WS-V.B step-1b → WS-V.B Phase 1 GT-encoding-fix (`c7c853e`). Cumulative empirical outcome: **207 → 59 divergences (-71% reduction)** across the arc.

**Arc trajectory (5 iteration layers).**
...
3. WS-V.A1 FA patch (`b7251e7`) — ~53-61 rows closed; baseline reaches the (inflated) 93-row plateau that drove WS-V.B opening.
4. WS-V.B step-1 + step-2 (VBD-1 falsified, no commit) — D3-class mechanism walkthrough derived from canonical-writer firings; +71-divergence regression in shadow falsified the hypothesis; S35 candidate emerged.
5. WS-V.B step-1b (`ca22851`) + WS-V.B Phase 1 (`c7c853e`) — S35-anchored cohort re-partition (35 Y + 10 Z + 10 W) + GT-encoding-fix (34 rows closed; H3 META-FINDING confirmed).

**H3 baseline-calibration META-FINDING (load-bearing for all future arcs).** WS-V.B step-1b H3 PARTIAL-CONFIRMATION revealed the diff baseline (post-WS-V.A1) over-counted pipeline defects by 34-45 rows for the entire iteration arc — GT ingester's `striker_after_rotation` encoding (lazy-rotation at next-over-start) mismatched the pipeline's eager-rotation convention (apply at X.6 before snapshot). Phase 1 GT-encoding-fix at `c7c853e` corrected the ingester semantic; validated baseline post-Phase-1 = **59 divergences** (vs inflated 93). All future predicted-flip tables compare against 59. The Y cohort (35 rows) + 1-row boundary cascade closed under Phase 1; Z cohort (10 rows) + W cohort (10 rows) remain as acceptable residue and Phase 4 catalogue item respectively. Operational corollary: any future arc that quotes a "93 baseline" is citing inflated pre-calibration data; rebase against 59.

**S34 PROMOTED to numbered methodology insight (5th-instance evidence).** Canonical: "Before opening step-N, spot-check step-(N-1)'s UNVERIFIED markers via 2-3 targeted code reads + trace inspection. ~10× cost reduction demonstrated across 5 instances (WS-L step-1b + WS-M step-1b + WS-V.A1 step-1b + WS-V.B step-1 + WS-V.B step-2 phase-3). Each instance prevented a falsifiable patch from shipping; cumulative saved patches = 5+ across iteration arc." S34 + S26-v2 jointly form the spot-check-discipline pair (S34 = pre-step-N spot-check at investigation phase; S26-v2 = pre-step-N spot-check at patch-verification phase).

**S35 PROVISIONAL PROMOTION (first-instance with promotion-on-second-instance trigger).** Canonical: "Mechanism walkthroughs at step-1 require empirical anchoring to actual write-site output, not derivation from canonical-writer firings. First-instance evidence: WS-V.B step-2 phase 3 parity check empirically falsified the VBD mechanism hypothesis from step-1, preventing a +71-row regression. The S33 parallel-comparison phase is the empirical anchor; without it, static-derived hypotheses can be wrong even when canonical-writer firings appear consistent. S34 catches wrong-layer hypotheses at investigation phase; S35 catches wrong-mechanism hypotheses at patch phase." Full S35 promotion deferred to second-instance corroboration.

**S33 second-instance footprint (no promotion).** Parallel-prompt parity discipline reinforced by WS-V.B step-2 empirical-parity falsification. Two instances total (WS-O.c canonical landing + WS-V.B step-2). Awaits independent third-instance for full promotion.

**Cumulative methodology footprint (this commit).** S33 (2nd-instance footprint) + S34 (PROMOTED) + S35 (PROVISIONAL PROMOTION). The S33+S34+S35 triad now operates as the empirical-anchoring discipline stack for mechanism investigation: S33 (parallel-comparison primitive) + S34 (spot-check before step-N) + S35 (empirical-anchor at step-1 mechanism walkthrough). All three were invoked across WS-V.B step-1b's audit trail; cumulative saved patches across the arc: 5+ (WS-O.b OA + WS-P P1 + WS-V.A1 D3-extension catch + WS-V.B step-1 D3-falsification + WS-V.B step-1 VBD-falsification at step-2 phase 3).

**Phase 4 catalogue additions.** (a) **WS-V.B.W** — 10-row Cohort W at mid-over 4.3, true pipeline-defect (striker_name vs stats internal inconsistency); requires step-1c instrumentation + step-2 patch (3-step iteration for 1.7% closure). (b) **WS-V.B.Z** — 10-row Cohort Z at mid-over 4.4, acceptable residue (may also be GT-encoding-related; needs re-verification post-Phase-1 if revisited).

**Methodology insights running total: 25 → 27 (S34 + S35 promoted).** S33 (2-instance footprint awaiting independent third) + S25 + S27 + replay-tool-fitness + scope-expansion-cascades-as-Phase-2-signal candidates remain awaiting respective promotion thresholds.

Full retrospective in `files/docs/investigations/workstream_v_b_step1b_empirical_anchor_residual_mechanism_reclassification.md` §11-§14 + `workstream_v_a1_score_commit_gate_over_rejection_root_cause.md` arc-close section.

---

## §17.12 WS-N PHASE-2-REFRAMED (Phase 2 reframe retrospective)

WS-N's static-investigation chain (step-1 + step-1b + step-1c + step-2 verification, four commits total: `3914f70` + `6c322c5` + `f8f11b1` + this Phase 2 reframe) surfaced six-layer S26-v2 cascade with ~10× scope growth from initial estimate to honest scope.

**Six-layer cascade timeline:**
1. Step-1 §4 NA initial estimate: **~5-10 LOC** (grep + script docstring).
2. Step-1b sub-mechanism (b) verdict: **~15-25 LOC** (sm.on_frame body scan).
3. Step-2 verification (prior; no commit): **~50-75 LOC** (BallEventDetector class disambiguation + WICKET-ATTRIB body read).
4. Step-1c Fork C MESSY-PARTIAL verdict: **~70-115 LOC** (`_pending_bcast_striker_key` site enumeration).
5. Step-2 verification (this; no commit): **~90-165 LOC** (end-to-end read surfacing closure-tangling).
6. Phase 2 reframe (this commit): scope honestly bounded at the static-investigation-converged estimate.

**Phase boundary discrimination signal candidate.** WS-N's six-layer cascade is empirically distinct from S26-v2's spot-check pattern. S26-v2 catches drift at single layer (~1-3 tool calls per layer); the cascade-as-Phase-2-signal candidate catches CUMULATIVE scope growth across multiple layers. If ≥4 cascading layers each grow scope ≥1.5× prior estimate, the workstream is architectural-refactor-class disguised as surgical-fix → Phase 2 reframe rather than Phase 1 push.

**WS-N status.** **PHASE-2-REFRAMED.** Active backlog. Canonical scoping at memo §1-§21. Execution gated on cross-module-fix verification cohort growth OR empirical-budget recovery.

**S26-v2 sixth-instance footprint at step-2 verification** reinforces the discipline's working-as-designed validation. Every spot-check across the cascade prevented a misspecified step-N commit; total saved across the cascade: ~60+ tool calls + multiple revert/refinement cycles + likely 1-2/5 empirical-budget consumption from validation cycles that would have failed for predictable reasons.

**Methodology insights running total: 25 (unchanged).** Scope-expansion-cascades-as-Phase-2-signal candidate logged at WS-N memo §21 awaiting second-instance for promotion.

**Standing arc-economics observation.** The S26-v2 framework operated at its design ceiling for WS-N — the methodology cap is not abandoned but actively defended by the static-investigation framework's discrimination. WS-N's Phase 2 reframe is itself an architectural-correctness outcome rather than a workstream failure.

Full retrospective in `files/docs/investigations/workstream_n_snapshotter_full_pipeline_integration.md` §19-§22.

---

## §17.11 WS-M ARC CLOSED (step-4 retrospective)

WS-M arc closed at step-4 with Shape 2 patch (`d2f0759`) landed + unit-verified + production confirmation deferred to natural session. Empirical-replay-tool inadequacy finding is the load-bearing artifact of the 1/5 budget consumption — mirrors WS-G insight #17/#18 pattern (budget consumption produces methodology insight, not wasted iteration).

**Arc statistics.** 4 commits (`cfbd2f5` step-1 + `c46fcb0` step-1b + `d2f0759` step-2 + this step-4) + 1 empirical replay (no commit). 1/5 empirical-budget consumed. L1.5 75 → 80 cases. Shape 2 patch shipped + 5/5 L1.5 unit cases PASS + no regression observed.

**Production confirmation deferral rationale.** Step-3 empirical attempt via `replay_captured_scout_trace.py` triggered mechanism-confirmation FAIL (PENDING-BALL-ORPHAN-FALLBACK-ENQUEUED count = 0). Root: SM-only replay tool bypasses over_mgr ↔ score_manager wiring (via `attach_score_manager`); the orphan-bind path never exercises in this replay class. The 12-run gap on `validate_dckkr_20260521_070545` exists in PRODUCTION traces because production pipeline (test_pipeline.py) wires over_mgr; the SM-only replay can't reproduce. Production verification deferred to natural session traces (budget-neutral re-run of shipped assertion).

**S26-v2 PROMOTED to numbered insight (candidate → standing).** Two-instance evidence: WS-L step-1b (10-call deviation discovery cost) + WS-M step-1b (1-call cohort-split discovery). ~10× cost reduction demonstrated. Canonical: "Before opening step-N, spot-check step-(N-1)'s UNVERIFIED markers via 2-3 targeted code reads + trace inspection. Deviation at this stage costs ~1-3 tool calls; deviation at step-N verification-1 costs ~10+ tool calls + memo writing + STOP."

**Replay-tool-fitness candidate first-instance footprint** (NOT yet promoted; defer naming). Statement: "Validate replay-tool fitness for the hypothesis fix-surface category BEFORE authorizing empirical replay budget consumption. Cross-module wiring fixes cannot be validated via SM-only replay tooling. Pre-screen at step-2 verification: confirm replay scope covers the fix surface's module-interaction graph." Awaits second-instance for promotion.

**Methodology insights running total: 24 → 25 (S26-v2 promoted).** S22 + S26 + S26-v2 + S28 now form the standing static-investigation cost-optimization framework. S25 + S27 + replay-tool-fitness candidates awaiting respective second-instance thresholds.

**Phase 1 second-pillar productive-work chain.** **COMPLETE.** WS-Per-Batter-Ledger → WS-L → WS-M (11 commits + 1 empirical across the chain). Cheap-wins phase has narrowed to its structural floor. Remaining Phase 1 admissible work: assertion-side WS-I-pattern reuse only (per S28 pre-screen). External-corroboration pivots retired per 2026-05-24 user direction (broadcast IS the corroboration); pipeline-plumbing arcs remain open for prioritization.

**Production-session-driven validation mode** is the new standing discipline. Natural production sessions accumulate traces; shipped assertions re-run against new traces budget-neutrally to confirm Sub-cohort A predicted closure.

**Phase 4 catalogue entries from WS-M arc:** (a) Sub-cohort A production-confirmation-pending — re-run on natural session traces. (b) Sub-cohort B 1-run residual — non-orphan-bind class, defer to natural-cohort-growth-triggered future investigation. (c) Sub-mechanism (b) all-bound + (c) queue-overflow remain uncovered — re-open WS-M-extension if natural sessions surface either.

Full retrospective in `files/docs/investigations/workstream_m_orphan_bind_pipeline_plumbing_investigation.md` §15-§20.
...
**External-corroboration framing retired (2026-05-24 user direction).** Any runtime dependency on third-party data sources (real-time Cricbuzz commentary, external audio feeds, etc.) is out of scope. Broadcast IS the corroboration. F1017 phantom-wicket + similar architecturally-known-defects either get fixed within the broadcast-reading framework (Scout primitive expansion, broadcast wicket-banner detection, broadcast at-the-crease cross-check, broadcast replay/slow-mo detection, broadcast audio analysis) or stay as standing limitations. Cricbuzz commentary fixture remains valid as offline regression-test ground truth; NOT a production data source. Prior KF Cricbuzz-corroboration Phase 2 deferral queue retired.
...
Each session contributes one or more transferable methodology insights that survive into the next session's discipline. **The discipline track record is itself a load-bearing artifact** — preserve it; document new insights as they accumulate. Cumulative running total: **27 insights** (#1-#16 prior + S16/S17/S18 step-4 + S19 step-5 + S20 step-8 + S21 step-5b + S22 step-9 arc-retirement + S23 WS-I + S26 WS-Surface-E + S28 WS-L + S26-v2 WS-M + S34 WS-V.B + S35 WS-V.B PROVISIONAL). Standing methodology framework now includes S33+S34+S35 empirical-anchoring discipline stack (parallel-comparison primitive + pre-step-N spot-check + step-1 mechanism walkthrough empirical-anchor) post WS-V arc close-out.

## §18 — Live match session (2026-05-24, KKR vs DC, IPL 70th match, session `live_20260524_185913`)

**Headline**: First full-broadcast live-capture session against a real IPL match (Cricbuzz match_id 152263). 12GB MPEG-TS + 907MB fragmented MP4 archive captured cleanly from UGREEN HDMI input. UI lock failed mid-match due to interacting defects — replay-from-recording is the recovery path. Live-match runbook + ball-by-ball logger + shutdown script committed; one code patch (`udp_frame_source.py` low-delay ffmpeg flags) shipped pre-emptively but didn't fully resolve the consumer-buffer-lag class.

### What landed (committed before/during the match)
- `files/scripts/ball_by_ball_logger.py` — async WS subscriber writing one TSV + JSONL row per state-signature change (signature = innings × overs × score × wickets × striker × non × bowler × this_over). Reconnects across pipeline restarts. Intended as the post-match diff substrate vs Cricbuzz commentary.
- `files/docs/LIVE_MATCH_RUNBOOK.md` — 6-terminal launch sequence (A=UI, B=recorder UDP 9998→.ts, C=pipeline, D=UGREEN sender with `-f tee` triple-sink, E=clip extractor, F=ball logger). Drain order, archive paths, restart procedures, post-match diff workflow.
- `scripts/shutdown_live_match.sh` — graceful auto-shutdown with SIGINT→TERM→KILL escalation, leaves UI up. Scheduled at 23:30 IST via `nohup sleep+exec`; fired on schedule.
- `files/eyes/capture/udp_frame_source.py` — added `+nobuffer`, `-flags low_delay`, `-max_delay 0` to the internal ffmpeg decoder subprocess (initial patch with `-probesize 32 -analyzeduration 0` reverted after empirical evidence of blank/garbage frames — those flags starve MPEGTS H264 parser).

### Archives (preserved at `files/logs/deliveries/live_20260524_185913/`)
| File | Size | Source |
|---|---|---|
| `match_live_20260524_185913.ts` | 12GB | UDP 9998 → ffmpeg `-c copy` recorder |
| `match_live_20260524_185913.mp4` | 907MB | Mac UGREEN sender `-f tee` mp4 sink |
| `scout_raw.jsonl` | 392 lines | Vision SCOUT raw VLM output |
| `logs/openscout-live_20260524_185913.jsonl` | 2497 lines | OpenScout sidecar |
| `../../ball_log/ball_log_live_20260524_185913.tsv` | 8 lines | Ball-by-ball signature changes |

### Defects observed during the live run (root-cause attribution pending replay analysis)

1. **D-LIVE-1 — Stale `match_state_cache.json` (pre-launch step skipped → SM regression-reject latch).** Runbook step 3 (`rm -f files/match_state_cache.json`) was not executed at T-30. Pipeline hot-resumed from a May 23 cache claiming DC=142/8 at 20.0 overs (innings-end state from a prior session). When live Vision reads landed showing DC=70/1, SM rejected them as score regressions and stayed latched. Fix mid-match: deleted cache + restart. **Pattern**: stale-cache-as-state-source defect; runbook's pre-clean step must be force-enforced (a single `git pull` doesn't clear this state). Candidate workstream: auto-clear cache if `saved_at` older than N hours.

2. **D-LIVE-2 — UDPFrameSource consumer-buffer lag (decoder accumulation over time).** Pipeline's internal ffmpeg subprocess (`udp_frame_source._ffmpeg_argv`) accumulated input buffer such that consumer (Python frame queue) trailed producer (UDP 9999 input) by 60-90s after ~10-15min of running. Recorder on UDP 9998 (`-c copy` no decode) stayed live throughout. Manual restart of Terminal C cleared the buffer for ~10-15min then re-accumulated. Low-delay patch (`+nobuffer -flags low_delay -max_delay 0`) helped but didn't fully eliminate. **`[UDP-STREAM-FROZEN]` warning fires** when 5+ consecutive consumer reads return byte-identical md5, but **no auto-recovery is wired** — only a logged warning. Candidate workstream: on FROZEN-streak ≥ N, kill+respawn the ffmpeg subprocess (already has `_kill_ffmpeg` + `_spawn_ffmpeg`, just needs a watchdog escalation).

3. **D-LIVE-3 — Vision SCOUT cadence too slow for live ball-by-ball (1 call per 7-10s).** AdaptiveSleep + pre-Scout pixel-diff + text-band-detection gates rejected most frames, calling Vision only every 7-10s even during active play. At T20 ball rates (~20-30s per delivery including breaks), single-delivery resolution requires Vision to fire 2-3× per ball; 1 call per 10s misses some balls entirely. **Live UI never reached steady-state ball-by-ball tracking** even when frames were fresh. Candidate workstream: tighten AdaptiveSleep ceiling during ACTIVE-PLAY classification; loosen pixel-diff threshold for ticker-region.

4. **D-LIVE-4 — Bad per-batter ledger latch (Vision misreading between-overs stats graphics).** OpenScout output at 15:18:33 read "MOST WICKETS BY SPINNERS TATA IPL 2026 KOLKATA KNIGHT RIDERS 34 RAJASTHAN ROYALS 30 GUJARAT TITANS 23 …" — a between-overs stats package. UI captured at user request showed `Miller 94(44), Patel 43(43)`, partnership 137(87) — mathematically consistent within itself but factually wrong (broadcast showed `AXAR 22(17), MILLER 4(5)`). Hypothesis: Vision SCOUT mis-parsed a stats-graphic team-row as a per-batter card row; SM's per-batter ledger latched on the misread; subsequent correct reads were rejected as regressions. **This is the same defect-class shape as WS-V.A1 (Phase 1 GT-encoding mismatch) but on the WRITE side, not the GT side.** Candidate workstream: graphic-context gating before per-batter card writes — if `frame_phase ∈ {advertisement, graphic, stats_package}`, suppress batter-card updates.

5. **D-LIVE-5 — Vision SCOUT mis-classifying live broadcast frames as ADVERTISEMENT.** Pipeline log at 20:31:30 IST showed `[F58 VISION] [SCOUT] ADVERTISEMENT` despite broadcast being on live play with scoreboard visible (per recording extract). Could be edge-case of camera-view classifier. Candidate workstream: cross-check `camera_view: ad` against `STRIP: <non-null>` — if strip parses, downgrade ad classification.

### What worked end-to-end
- UGREEN HDMI capture after correct device index identified (video=1, audio=5; FaceTime camera = device 0 was the initial wrong default).
- ffmpeg `-f tee` muxer with single libx264 encode + three sinks (UDP 9999, UDP 9998, fragmented mp4). The pre-runbook 3-output form had been silently mis-encoding the mp4 by mixing `-c:v libx264` with `-c copy` on the same input.
- Recorder maintained ~6.0 Mbps continuously for ~4 hours = 12GB clean .ts.
- Ball-by-ball logger reconnect-on-restart was correct (logger stayed up across multiple pipeline restarts; rows appeared whenever SM emitted a signature change).
- Scheduled shutdown fired exactly at 23:30 IST with graceful SIGINT→TERM cascade.

### Open methodology insight (candidate, first instance)

- **S36 (candidate, first-instance, awaiting replay-confirmation)** — **Recording-as-source-of-truth allows post-match decoupling of capture defects from pipeline defects.** When a live session surfaces interacting defects (cache-stale + decoder-lag + cadence-thin + ledger-latch + ad-misclassify), the live UI is unfit for diagnosis but the .ts/.mp4 recording is. Replay the .ts via `FRAME_SOURCE=file` and the entire defect set becomes deterministic: cache-stale collapses (rm cache once), decoder-lag collapses (file path has no decoder buffering at consumer side), cadence-thin still applies (it's pipeline-internal) but is now reproducible per-replay, ledger-latch is reproducible, ad-misclassify is reproducible. **Operational corollary**: a multi-hour broadcast capture is a substrate for N replays; each replay isolates one defect with single-IV discipline. Captured-replay regression-suite extends from synthetic fixtures to genuine-broadcast. Awaiting promotion to standing methodology on second-instance evidence.
