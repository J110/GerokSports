...
**Session ledger.** **55 cumulative commits** across five consecutive sessions on `derive-not-detect` lineage: 16 B-η + 8 C19-C24 + 8 C26-C31 + C32 docs + 10 §15 arc (9 feat + 1 docs) + 8 Workstream G + 1 pre-merge gitignore + 1 merge commit + 3 Workstream H (step-1 memo + step-2 P1 patch + step-4 docs close-out). Layer 1.5 (36 ledger balls + 6 cross-field pairing + 3 D1 bowler-dispatch + 3 D2 WICKET-ATTRIB + 6 pending-cascade + 6 WS-H wickets_regressed guard = **60 cases**) + Layer 2 (30 balls) held on every commit. Derivation unit tests at 48/48. **Empirical-falsification budget: 3/5 remaining** (4/5 → 3/5 consumed by WS-H step-3 empirical validation on `validate_ws_h_step3_20260523_164642`; load-bearing artifact = S16 + S17 + S18 sub-findings per WS-H memo §10, mirroring WS-G's "budget consumption produced insight #17 + #18" framing). **WS-H primary objective CLOSED** — D-post-FoW-striker 20 → 2 (90% reduction, exceeds 5-10 prediction per insight #18 scope). Two residuals OPEN as follow-on workstreams (WS-H step-5 F939 sibling-asymmetry per S16; WS-H step-5b F679 cascade-drain trail-dependency per S17).
...
The PendingCascade lifecycle is regression-detected by `trace_eta_post_wicket_cascade_drains` (the η-bundle's single assertion). Baseline shift across the WS-H step-3 trace: `validate_surface_b_121222` (pre-P1) = 2 enqueues + 2 WIPED terminals (PASS); `validate_ws_h_step3_20260523_164642` (post-P1) = 2 enqueues + 2 **DRAIN-FIRED** terminals (F680, F858) + 0 WIPED + 0 EXPIRED (PASS). Cascade lifecycle now terminates in resolution rather than wipe-by-cold-start.

**Workstream G + H primary objectives CLOSED.** WS-G closed the lifecycle gap at `50af67e` (Surface B; WIPED-BY-COLD-START tags wire up). WS-H closed the D-post-FoW-striker primary objective at WS-H step-3 (P1 `ef0860d`: `_detect_innings_change` wickets_regressed branch gets team-change-corroboration guard mirroring 2026-05-19 score_reset_from_progress hardening). Empirical surface count: D-post-FoW-striker **20 → 2** (90% reduction; exceeds 5-10 prediction per insight #18 scope-separation). Composite-fix closure as side effect: γ-bowler-w FAIL × 4 → PASS (cascade DRAIN-FIRED enables D1's resolved bowler identity to propagate to the wicket-attribution site; S18 in WS-H memo §10).

**Two follow-on workstreams OPEN.**
- **WS-H step-5** (S16 surface) — F939 single-frame KKR-misread accepted via inherited consensus-asymmetry from `_team_changed` precedent. Remediation: consensus-gate `_team_changed` at `score_manager.py:4417-4419` to parity with `batting_team_changed :4337-4385`; closes both `wickets_regressed` (P1) AND `score_reset_from_progress :4423` consumers simultaneously. See `files/docs/investigations/workstream_h_cold_start_reentry_root_cause.md` §9.1 + §11.1.
- **WS-H step-5b** (S17 surface) — F679 `trace_gamma_fow_name_matches_striker_at_wicket` regression (PASS → FAIL × 1) caused by cascade DRAIN-FIRED at F680 reshuffling prev_striker bookkeeping at the wicket-commit frame. Remediation: make γ-fow-name prev_striker reconstruction drain-aware. See WS-H memo §9.2 + §11.2.

Both are independent surfaces; operator picks priority. The architectural-fence invariants from WS-G + §15 are unchanged.
