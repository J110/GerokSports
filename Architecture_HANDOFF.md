...
**Last update.** 2026-05-24 (**WS-V.B ARC CLOSED — Phase 1 GT-encoding-fix landed** at ingester `files/scripts/ingest_cricbuzz_ground_truth.py:547-553/686-691`; end-of-over rotation switched from lazy-at-next-over-start to eager-before-snapshot-at-X.6 to match pipeline convention; 34 rows closed (Y cohort over-end 0.6/1.6/2.6/3.6); 59-row validated baseline replacing the 93-row inflated baseline; **H3 META-FINDING CONFIRMED** — diff baseline was over-counting pipeline defects by 34 rows for entire post-WS-V.A1 iteration arc; cumulative WS-V arc trajectory: 207 → 59 divergences (~71% closure across 5 layers from WS-V step-1 → WS-V.B Phase 1); S33 second-instance footprint + S34 fifth-instance footprint + **S35 provisional-promotion** (empirical-anchor to actual snapshot-write output saved +71-divergence regression at WS-V.B step-2 VBD-1 falsification); WS-V.B.W (10-row cohort W at 4.3 — true pipeline cricket-defect) DEFERRED to Phase 4 catalogue; cohort Z (10 rows at 4.4 — GT-vs-pipeline mid-over convention mismatch) DECLARED ACCEPTABLE RESIDUE).
