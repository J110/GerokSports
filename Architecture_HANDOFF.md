...
**Last update.** 2026-05-24 (WS-N **PHASE-2-REFRAMED** — full-pipeline snapshotter integration scoped at ~90-165 LOC via six-layer S26-v2 cascade demonstrating ~10× scope growth from initial estimate; active Phase 2 backlog item; runbook fix `3e8ac11` SM-only caveat preserves Phase 1 operational coverage; scope-expansion-cascades-as-Phase-2-signal candidate logged at WS-N memo §21 awaiting second-instance; standing discipline unchanged at 1/5 budget methodology cap proximity).
...
|---|---|---|
| **WS-N — Full-pipeline snapshotter integration** | (a) ≥3 workstreams blocked on cross-module-fix verification via runbook OR (b) empirical-budget ≥3/5 | `files/docs/investigations/workstream_n_snapshotter_full_pipeline_integration.md` §1-§21 (six-layer S26-v2 cascade analysis + Fork A/B/C/D cost-benefit + Fork C extractability classification + `_pending_bcast_striker_key` per-frame state mirror requirements) |

**Phase 2 backlog discipline.** Each Phase 2 entry has a canonical scoping artifact (typically the static-investigation memo that surfaced the Phase 2 reframe). When Phase 2 execution triggers, no fresh step-1 investigation needed — the scoping artifact IS the entry data. Multi-commit execution plans assumed for Phase 2 work; budget recovery to ≥3/5 prerequisite ensures the empirical-validation cycles can absorb expected cost.

**Operational mitigation during Phase 1.** Runbook SM-only caveat at `3e8ac11` documents the WS-N limitation explicitly. Production-session-driven validation mode handles cross-module fix verification need at zero budget cost via natural session traces + shipped assertion re-run.
...
4. **S22 + S28 + replay-tool-fitness-candidate static-investigation-cost-optimization framework** (post-WS-H + WS-Per-Batter-Ledger + WS-L + WS-M) — workstream-opening + step-opening + empirical-validation cost-optimization framework.

**Budget at 1/5 — methodology cap proximity.** Standing discipline shifts to **production-session-driven validation mode.** Natural production sessions accumulate traces; shipped assertions (trace_alpha_batter_runs_sum + γ-bundle + η-bundle + α-bundle) re-run against new traces at zero budget cost. Sub-cohort A's predicted closure (18-run total) verifiable budget-neutrally on next natural session.

**Phase 1 second-pillar productive-work chain.** **COMPLETE.** WS-Per-Batter-Ledger → WS-L → WS-M. 11 commits + 1 empirical + 1/5 budget consumed. Cohort discriminators shipped + 2 production patches landed. Cheap-wins phase has narrowed to structural floor.

**Remaining Phase 1 admissible work:** assertion-side WS-I-pattern reuse only (per S28 pre-screen). Pipeline-plumbing arcs (WS-D Layer 1a, WS-F bowler-misattribution) + external-corroboration architectural pivots (KF Cricbuzz-corroboration) are **unaffordable at 1/5 budget** until budget recovery.
