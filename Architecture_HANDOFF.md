...
**Last update.** 2026-05-24 (**WS-Q ARC FULLY CLOSED — step-3 arc close-out commit**; multi-commit refactor delivered as predicted; cumulative WS-Q trajectory **59 → 34 divergences (-42% closure at step-2c; -83.6% from original 207-row baseline)**; **S9-strict PROMOTED to numbered methodology insight** (3rd-instance evidence: F1 cascade-closure + S18 composite-fix + WS-Q canonical-store-flag-flip; architectural cure closes multiple defect classes simultaneously — E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline closed simultaneously at step-2c `88fe41c`); **FA forward-compat empirical validation** landed as candidate insight (first-instance: WS-V.A1 step-2 §10 prediction confirmed via cross-fixture audit at `4e3b3d3` — `PREV-SCORE-ANCHOR-APPLIED` count = 0 across 4 captures + 2,472 trace records post-step-2c flag flip; full promotion deferred to second-instance); **architectural-cure-over-defensive-iteration economics** landed as candidate insight (first-instance: WS-Q -25 rows / ~107 LOC vs prior 3 defensive no-ops WS-O OA + WS-P P1 + WS-U V2 = 0 rows / ~150-200 LOC similar effort). Validated baseline NOW = **34** (was 59 post-WS-V Phase 1; before that inflated 93). All future predicted-flip tables compare against 34. Methodology insights running total **27 → 28** (S9-strict promoted) + 2 candidates. Phase 4 catalogue updates: E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline RETIRED via WS-Q cascade closure; S12 (eyes/main.py threading) ADDED from WS-Q step-2d deferral.
...
## Foundational infrastructure queue (active; immediate-execution)

Budget discipline retired per user direction 2026-05-24. WS-N re-tiered out of Phase 2 backlog as foundational-infrastructure work; immediate-execution.

| Workstream | Status | Scoping artifact |
|---|---|---|
| **WS-N — Full-pipeline snapshotter integration** | Immediate execution; multi-commit arc expected | `files/docs/investigations/workstream_n_snapshotter_full_pipeline_integration.md` §1-§21 (six-layer S26-v2 cascade analysis + Fork A/B/C/D scope analysis + Fork C extractability classification + `_pending_bcast_striker_key` per-frame state mirror requirements) |

**Foundational infrastructure discipline.** Validation methodology fixes are not subject to the cost-discipline rules that applied to defect-class workstreams under the prior budget framework. Each entry has a canonical scoping artifact (typically the static-investigation memo that surfaced the scope). When execution triggers, no fresh step-1 investigation needed — the scoping artifact IS the entry data. Multi-commit execution plans assumed.

**Operational mitigation during execution.** Runbook SM-only caveat at `3e8ac11` documents the current WS-N limitation. Production-session-driven validation mode handles cross-module fix verification needs in the interim via natural session traces + shipped assertion re-run.

---

## Architectural fence — post-WS-V arc close-out (2026-05-24)

WS-V arc closed at combined step-4 docs commit (this) after 5-layer iteration trajectory: WS-V step-1 → WS-V.A → WS-V.A1 FA (`b7251e7`) → WS-V.B step-1 + step-2 (VBD-1 falsified, no commit) → WS-V.B step-1b (`ca22851`) → WS-V.B Phase 1 GT-encoding-fix (`c7c853e`). Cumulative empirical outcome: **207 → 59 divergences (-71% reduction)** across the arc.

**Standing arc-economics — validated baseline = 59 (NOT inflated 93).** WS-V.B step-1b H3 PARTIAL-CONFIRMATION revealed the diff baseline (post-WS-V.A1) over-counted pipeline defects by 34-45 rows for the entire iteration arc — GT ingester's `striker_after_rotation` encoding (lazy-rotation at next-over-start) mismatched the pipeline's eager-rotation convention (apply at X.6 before snapshot). Phase 1 GT-encoding-fix at `c7c853e` corrected the ingester semantic; **validated baseline post-Phase-1 = 59 divergences (vs inflated 93)**. **All future predicted-flip tables compare against 59, not inflated 93.** Operational corollary: any future arc that quotes a "93 baseline" is citing inflated pre-calibration data; rebase against 59. The Y cohort (35 rows) + 1-row boundary cascade closed under Phase 1; Z cohort (10 rows) + W cohort (10 rows) remain as acceptable residue and Phase 4 catalogue item respectively.

**S34 PROMOTED to numbered standing methodology (5th-instance evidence).** Canonical statement: "Before opening step-N, spot-check step-(N-1)'s UNVERIFIED markers via 2-3 targeted code reads + trace inspection. ~10× cost reduction demonstrated across 5 instances (WS-L step-1b + WS-M step-1b + WS-V.A1 step-1b + WS-V.B step-1 + WS-V.B step-2 phase-3). Each instance prevented a falsifiable patch from shipping; cumulative saved patches = 5+ across iteration arc." S34 + S26-v2 jointly form the spot-check-discipline pair (S34 = pre-step-N spot-check at investigation phase; S26-v2 = pre-step-N spot-check at patch-verification phase). S34 promotion supersedes the prior "S26-v2 standalone" framing — both are now standing infrastructure alongside §15 + S21 + S23 + S26 + S28.

**S9-strict PROMOTED to numbered methodology insight (3rd-instance evidence, WS-Q arc close-out).** Canonical statement: "Single architectural fix closing multiple defect classes simultaneously. Three-instance evidence at promotion: F1 cascade-closure (cold-start striker fix closed N classes); S18 composite-fix (D1+P1 closed γ-bowler-w residue); WS-Q canonical-store-flag-flip (closed E2-phantom-runs + Per-batter-ledger-drift + phantom-in-pipeline simultaneously per step-2c `88fe41c`). **Pattern: when defensive iterations saturate against persistent residue, root-architectural-fix is the next layer to investigate — not more defensive layers.**" S9-strict joins §15 + S21 + S23 + S26 + S26-v2 + S28 + S34 + S35 as standing methodology infrastructure.

**Standing arc-economics — WS-Q architectural-cure-over-defensive-iteration evidence (candidate insight; first-instance via WS-O.c residue closure).** WS-Q's 3-commit arc (step-2a + 2b + 2c; ~107 LOC production logic) closed -25 rows on WS-O.c residue where prior 3 defensive attempts (WS-O OA + WS-P P1 + WS-U V2; combined ~150-200 LOC similar effort) closed 0 rows. Plus FA forward-compat empirical validation (candidate; first-instance): WS-V.A1 step-2 §10 prediction → cross-fixture audit at `4e3b3d3` confirmation (FA count = 0 across 4 captures + 2,472 trace records post-step-2c flag flip). Both candidates await second-instance for full promotion. Trigger pattern: "When defensive iterations saturate (3+ consecutive empirical no-ops within same defect class), pivot to architectural root-cause identification, not additional defensive layers." Documented at `files/docs/investigations/workstream_q_score_state_unification_refactor.md` §11-§15.

**S35 PROVISIONAL PROMOTION (first-instance with promotion-on-second-instance trigger).** Canonical statement: "Mechanism walkthroughs at step-1 require empirical anchoring to actual write-site output, not derivation from canonical-writer firings. First-instance evidence: WS-V.B step-2 phase 3 parity check empirically falsified the VBD mechanism hypothesis from step-1, preventing a +71-row regression. The S33 parallel-comparison phase is the empirical anchor; without it, static-derived hypotheses can be wrong even when canonical-writer firings appear consistent. S34 catches wrong-layer hypotheses at investigation phase; S35 catches wrong-mechanism hypotheses at patch phase." Full S35 promotion deferred to second-instance corroboration; provisional promotion treated as standing methodology at this commit.

**Five-fence standing methodology infrastructure (post-WS-V).** Five peer fences now constrain investigation methodology:
1. **§15 canonical write-path fence** (post-§15 arc) — pipeline-side correctness fence; sole canonical write paths for striker / wicket / this_over.
2. **S21 + S23 two-shape assertion-side family** (post-WS-I) — assertion-side discipline; canonical-resolution graceful-degrade + schema-precondition graceful-skip.
3. **S26 + S26-v2 + S34 multi-layer falsification + spot-check framework** (post-WS-K + WS-L + WS-M + WS-V) — methodology fence; gate-2 audit must classify signal-source provenance + pre-step-N spot-check before commitment.
4. **S22 + S28 static-investigation + pre-screen framework** (post-WS-H + WS-Per-Batter-Ledger + WS-L + WS-M) — workstream-opening + step-opening quality framework.
5. **S33 + S35-PROVISIONAL empirical-anchoring discipline stack** (post-WS-V) — mechanism-investigation fence; step-1 walkthroughs anchored to actual write-site output; parallel-comparison primitive before any flip.

**H3 baseline-calibration discipline (load-bearing).** Differential-testing baselines themselves require empirical calibration before being trusted as defect counts. The 93 → 59 calibration is a one-off application of S35-anchored methodology to the differential-testing layer (rather than the pipeline-internal mechanism layer). Future workstreams that open a new differential-testing harness should apply the same discipline: spot-check the harness's encoding conventions against the pipeline's write conventions BEFORE quoting baselines as authoritative defect counts.

**Phase 4 catalogue additions (post-WS-V).** (a) **WS-V.B.W** — 10-row Cohort W at mid-over 4.3, true pipeline-defect (striker_name vs stats internal inconsistency); requires step-1c instrumentation + step-2 patch (3-step iteration for 1.7% closure). (b) **WS-V.B.Z** — 10-row Cohort Z at mid-over 4.4, acceptable residue (GT-vs-pipeline mid-over convention mismatch; may also be GT-encoding-related; needs re-verification post-Phase-1 if revisited). Other Phase 4 items (S23-corollary + S23-extension + §12 catalogue extensions + C20b + WS-N N1.3) unchanged.
...
**Next session: WS-Q score-axis step-2a** — foundation refactor behind `USE_SM_CANONICAL_SCORE=0` (chartered next step in canonical-API refactor). WS-V.A1 step-4 close-out retired into this commit; FA arc completeness load-bearing piece now in standing docs. The architectural-fence invariants from WS-G + §15 + WS-H + WS-I + WS-V are unchanged.

---

## Architectural fence — post-Workstream-M retirement (2026-05-23)

WS-M arc CLOSED at step-4. Shape 2 patch (`d2f0759`) at `score_manager.py:1436-1462` consumer-side fallback bind shipped + unit-verified (5/5 L1.5 cases) + no regression. Production confirmation deferred to natural session traces. The arc surfaced two methodology-significance findings: S26-v2 promotion (pre-step-N spot-check) + replay-tool-fitness candidate first-instance footprint.

**S26-v2 PROMOTED to numbered insight.** Canonical: "Before opening step-N, spot-check step-(N-1)'s UNVERIFIED markers via 2-3 targeted code reads + trace inspection. ~10× cost reduction demonstrated across WS-L step-1b + WS-M step-1b instances."

**Replay-tool-fitness candidate (NOT yet promoted; single-instance footprint).** Statement: "Validate replay-tool fitness for the hypothesis fix-surface category BEFORE authorizing empirical replay budget consumption. Cross-module wiring fixes cannot be validated via SM-only replay tooling. Pre-screen at step-2 verification: confirm replay scope covers the fix surface's module-interaction graph." First-instance: WS-M step-3 mechanism-confirmation FAIL on SM-only replay vs over_mgr-wired Shape 2.

**Architectural-fence + fix-surface-fence + signal-source-fence + methodology-cost-fence (post-WS-M).** Four peer fences now constrain investigation methodology:
1. **§15 canonical write-path fence** (post-§15 arc) — pipeline-side correctness fence.
2. **S21 + S23 two-shape assertion-side family** (post-WS-I) — assertion-side discipline.
3. **S26 + S26-v2 + S27-candidate multi-layer falsification framework** (post-WS-K + WS-L + WS-M) — methodology fence.
4. **S22 + S28 + replay-tool-fitness-candidate static-investigation framework** (post-WS-H + WS-Per-Batter-Ledger + WS-L + WS-M) — workstream-opening + step-opening + empirical-validation quality framework (cost-optimization framing retired per user direction 2026-05-24; framework retained as quality discipline).

**Budget discipline retired per user direction 2026-05-24.** Production-session-driven validation remains useful as a verification mode (natural session traces re-run shipped assertions at no additional capture cost) but is no longer treated as a budget-conservation mechanism. WS-N foundational-infrastructure execution is the immediate next move.

**Phase 1 second-pillar productive-work chain.** **COMPLETE.** WS-Per-Batter-Ledger → WS-L → WS-M. 11 commits + 1 empirical replay across the chain. Cohort discriminators shipped + 2 production patches landed. Validation methodology is now the constraint; WS-N closes it.

**Remaining work after WS-N:** ship all known fixes against the validated snapshotter to reach healthy-pipeline state where fresh replay surfaces no old bugs. Includes pipeline-plumbing arcs (WS-D Layer 1a, WS-F bowler-misattribution) and Phase 4 catalogue items. External-corroboration architectural work retired per 2026-05-24 user direction (broadcast IS the corroboration).

**Phase 4 catalogue queue:** (a) Sub-cohort A production-confirmation-pending; (b) Sub-cohort B 1-run residual (non-orphan-bind class); (c) S23-corollary multi-wicket-per-frame undercount; (d) S23-extension `_final_extras_total` silent-stricter-bound; (e) §12 dual-state-write 4th instance; (f) C20b surface_pair populate.

**Methodology insights running total: 24 → 25 (S26-v2 promoted).** S25 + S27 + replay-tool-fitness candidates awaiting respective second-instance promotion thresholds.

---

## Architectural fence — post-Workstream-K retirement (2026-05-23)

WS-K state-machine layer investigation closed at step-1 (commit `8f79b03`) with universal-candidate-exhaustion. **All 5 within-Phase-1-scope state-machine candidates (KA / KB / KC / KD / KE) statically falsified at gate 1.** Architectural finding: the §15 canonical write-path fence guarantees that every state-machine signal (striker pointer + partnership + bowler tracker + batting-card status + bat-slot resolution + this_over) flows through canonical write paths from strip OCR. When OCR is noisy at the phantom frame, ALL pipeline-internal state derived from that OCR is noisy in the same way. **The §15 fence is correctness-preserving (no non-canonical writes) but discriminator-limiting (no signal can exceed strip OCR's discriminative capacity).** Multi-layer convergent falsification across Surface E (detection) + WS-K (state-machine) confirms F1017 as architectural-known-defect at every pipeline-internal layer.

**S27 candidate — Shared-signal-source structural barrier (NOT yet promoted; single instance).** Statement: when sequential investigation layers all derive their signals from a shared upstream source, falsifications at any layer propagate upward into all downstream layers. The structural barrier is the shared source itself; investigation layers cannot exceed its discriminative ceiling. **Distinction from S26 (peer):** S26 is the process-level discipline ("more static layers → higher confidence in surviving conclusions"); S27 is the structural-cause peer (WHY do multi-layer falsifications converge? Because layers share an upstream source whose discriminative capacity bounds all downstream layers). Together: S26 (when applied) + S27 (why convergence emerges) jointly mature the multi-layer-investigation discipline into a standing audit framework alongside §15 + S21 + S23 + S26.

**Architectural-fence + fix-surface-fence + signal-source-fence (post-WS-K).** Three peer fences now constrain investigation methodology:
1. **§15 canonical write-path fence** (post-§15 arc) — pipeline-side correctness fence; sole canonical write paths for striker / wicket / this_over.
2. **S21 + S23 two-shape assertion-side family** (post-WS-I) — assertion-side discipline; canonical-resolution graceful-degrade + schema-precondition graceful-skip.
3. **S26 + S27-candidate multi-layer falsification framework** (post-WS-K) — methodology fence; gate-2 audit must classify signal-source provenance; predict shared-source-barrier falsification before next-layer commitment.

**External-corroboration framing retired (2026-05-24 user direction).** Broadcast IS the corroboration; any runtime dependency on third-party data sources (real-time Cricbuzz commentary, external audio feeds, etc.) is out of scope. F1017 phantom-wicket + similar architecturally-known-defects either get fixed within the broadcast-reading framework (Scout primitive expansion, wicket-banner detection, at-the-crease cross-check, replay/slow-mo detection, audio analysis) or stay as standing limitations. `files/scripts/ingest_cricbuzz_ground_truth.py` retains its role as offline regression-fixture ingester ONLY; not a production data source. Prior KF Cricbuzz-corroboration Phase 2 deferral retired.

**Trace assertion library baseline (UPDATED post-WS-Per-Batter-Ledger).** `trace_alpha_batter_runs_sum` added at `110026d` (Shape B schema-precondition + ≥70% BAT-DELTA coverage floor). Baseline outcomes across on-disk traces: LIVE PASS×4 (2 sum-match + 2 coverage-skip) + LIVE FAIL×4 (gaps 12 + 3 + 3 + 1 runs on validate_dckkr_* + validate_20260513_194442) + REPLAY SKIP×49 (no ui_after → Shape B). PHANTOM-WICKET-SUSPECT tag remains NOT in KNOWN_TAGS (Surface E Change 2 fully reverted at `061c77d` per Option X-naked authorization). γ-w-symbol post-WS-I baseline preserved. The 4 LIVE-FAILs surfaced are the empirical anchor for WS-L HC observability-completeness audit (companion commit this session).

**Phase 1 second-pillar slot.** RE-OPENS for re-selection. Recent-Overs / B-γ classified as Phase 3 UI render layer scope per HANDOFF line 725 standing classification ("out of scope for SM/pipeline workstream") — NOT a Phase 1 pipeline candidate. Recommended next-session: **§11.3 item 4 (Per-batter-ledger conservation)** OR WS-D §3.5 Layer 1a (HE survivor from C29b — pipeline-side, S12 plumbing concern, 1-2/5 budget) OR WS-F bowler-misattribution.

**Methodology insights running total: 23 (unchanged — S27 stays candidate).** S26 promoted at `a4f91f5`; S27 awaits second-instance confirmation; S25 awaits second-instance confirmation; S24 awaits third-instance confirmation.

---

## Architectural fence — post-Workstream-Surface-E retirement (2026-05-23)

WS-Surface-E retires without UI closure delivered. F1017 phantom-wicket on `validate_dckkr_20260521_155356` cataloged at `surface_pair_defect_class_family.md` §2.10 as architectural-known-defect. **No detection-layer fix preserves cricket-truth without false-negative regression** within the current Scout primitive set — the OCR-noise envelope around wicket events is uniform across phantom and genuine cohort (Rana F679 has prior regression F673 wkts=3→1; Nissanka F855 has prior regression F853 wkts=8→2; phantom F1017 has prior regression F1015 wkts=6 → F1016 wkts=4 — same shape, same K-window depth).

**Trace assertion library baseline (UNCHANGED from post-WS-I closure).** PHANTOM-WICKET-SUSPECT tag NOT in KNOWN_TAGS (Change 2 fully reverted at `061c77d`); no observability hedging preserved at the falsified detection-layer.

**S26 — Static-investigation-rounds compound (PROMOTED to numbered insight).** Each layer of static analysis can surface findings that falsify the prior layer's strategic framing. Three-instance evidence: (1) WS-C29b step-1 source-modality blocker falsification BEFORE any patch (saved 1/5 + commit churn); (2) WS-Surface-E step-2.5 HA' falsification AFTER step-2 commit `65ef9c9` (incurred revert cost `061c77d`); (3) WS-Surface-E step-2.6 cricket-physics-gate falsification via OCR-noise-uniformity finding (prevented Option Y patch). **Operational corollary (load-bearing for next workstreams): "gate-4 incomplete = STOP, do not proceed to patch-and-verify-empirically-later."** S22 (workstream-scope) + S26 (intra-workstream per-layer scope) jointly mature the static-investigation-first methodology into the standing audit framework.

**Phase 1 second-pillar slot.** RE-OPENS for re-selection. Surface E retirement consumed the slot without delivery. Recommended next-session candidate: **§11.3 item 5 (Recent-Overs partial-render)** — possibly assertion-side per WS-I pattern reuse; if confirmed, delivers budget-neutral UI closure mirroring WS-I economics.

**F1017 architectural-known-defect future-investigation prerequisites.** Closing requires (a) new Scout primitive providing discriminating signal (e.g., FoW-graphic overlay parsing); OR (b) ML classifier trained on labeled phantom-vs-genuine cohort; OR (c) operator-side post-hoc retraction workflow. None in Phase 1 scope.

**Methodology insights running total: 22 → 23 (S26 promoted).** S25 (strategic-framing-vs-source-modality) remains candidate at 2 instances awaiting third-instance confirmation.
