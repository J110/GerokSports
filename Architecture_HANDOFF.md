...
**Remaining work after WS-N:** ship all known fixes against the validated snapshotter to reach healthy-pipeline state where fresh replay surfaces no old bugs. Includes pipeline-plumbing arcs (WS-D Layer 1a, WS-F bowler-misattribution), external-corroboration architectural work (KF Cricbuzz-corroboration) per concrete need surfaced by the validated diff report, and Phase 4 catalogue items.
...
**KF Cricbuzz-corroboration Phase 2 architectural-pivot deferral.** Re-opening prerequisites enumerated at `workstream_k_phantom_wicket_state_machine_investigation.md` §14 — cohort growth OR Scout primitive expansion OR independent strategic justification. `files/scripts/ingest_cricbuzz_ground_truth.py` already ingests Cricbuzz commentary for post-hoc regression-fixture purposes; real-time wiring at `apply_wicket_event` site requires Phase 2 scoping.
